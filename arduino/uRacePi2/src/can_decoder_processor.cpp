...
float evora_wheelspeed_mph(const uint32_t raw) {
  if (raw == 0x3FFF) {
    return 0.0f;
  }
  return raw * 0.01736f;
  ...
  switch(frame->identifier) {
    ...
    case 0x0A2:
      if (dlc >= 6 ) {
        ...
        rc_set_data(RC_META_WHEEL_SPEED_LF, evora_wheelspeed_mph(lf));
        rc_set_data(RC_META_WHEEL_SPEED_RF, evora_wheelspeed_mph(rf));
    ...
    case 0x0A4:
      if (dlc >= 5 ) {
        ...
        rc_set_data(RC_META_WHEEL_SPEED_LR, evora_wheelspeed_mph(lr));
        rc_set_data(RC_META_WHEEL_SPEED_RR, evora_wheelspeed_mph(rr));
    ...
    case 0x303:
      ...
      if (dlc >= 7){
        ...
        // float long_accel = (long_raw - 2049) * 20385.0f / 100000.0f / 128.0f;

        // Lateral acceleration (12-bit, bytes 4-5)
        uint16_t lat_raw = ((uint16_t)data[4] << 4) | (data[5] >> 4);
        float lat_accel = (lat_raw - 2049) * 20385.0f / 100000.0f / 128.0f;
